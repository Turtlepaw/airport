...
export default function PlcUpdateProgress() {
  ...
  const [keyJson, setKeyJson] = useState<KeyData | null>(null);
  const [emailToken, setEmailToken] = useState<string>("");
  const [_updateResult, _setUpdateResult] = useState<string>("");
  const [_showDownload, _setShowDownload] = useState(false);
  ...
  const handleTokenSubmit = async () => {
    ...
    try {
      ...
      _setUpdateResult("PLC update completed successfully!");
      ...
      _setUpdateResult(error instanceof Error ? error.message : String(error));
