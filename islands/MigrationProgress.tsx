...
export default function MigrationProgress(props: MigrationProgressProps) {
  ...
  const [_retryAttempts, setRetryAttempts] = useState<Record<number, number>>(
    {},
  );
  const [showContinueAnyway, setShowContinueAnyway] = useState<
    Record<number, boolean>
  >({});
  const [migrationClient] = useState(() => new MigrationClient());

  const callbacks: MigrationCallbacks = {
    onStepUpdate: (_stepIndex: number, _step: MigrationStep) => {
      setSteps([...migrationClient.getSteps()]);
    },
    onStateUpdate: (state: MigrationState) => {
      setMigrationState(state);
    },
    onRetryUpdate: (stepIndex: number, attempts: number) => {
      setRetryAttempts((prev) => ({ ...prev, [stepIndex]: attempts }));
    },
    onShowContinueAnyway: (stepIndex: number, show: boolean) => {
      setShowContinueAnyway((prev) => ({ ...prev, [stepIndex]: show }));
    },
    onIdentityTokenRequired: () => {
      // Token input is already handled in the UI
    },
    onMigrationComplete: () => {
      // Migration complete - the UI will show the completion message
    },
  };
  ...
  useEffect(() => {
    ...
    setSteps([...migrationClient.getSteps()]);
    migrationClient.startMigration(migrationParams, callbacks);
  }, []);

  const handleIdentityMigration = async () => {
    if (!token) return;
    await migrationClient.submitIdentityToken(token, callbacks);
  };

  const retryVerification = async (stepNum: number) => {
    await migrationClient.retryVerification(stepNum, callbacks);
  };

  const continueAnyway = (stepNum: number) => {
    migrationClient.continueAnyway(stepNum, callbacks);
  };
